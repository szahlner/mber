...
parser.add_argument('--v-ratio', type=float, default=0.95, metavar='N',
                    help='virtual ratio (default: 0.95)')
...
parser.add_argument('--per-alpha', type=float, default=0.4, metavar='N',
                    help='PER alpha parameter (default: 0.4)')
parser.add_argument('--per-beta', type=float, default=0.6, metavar='n',
                    help='PER beta parameter (default: 0.6)')
parser.add_argument('--gan', action="store_false",
                    help='use GAN (default: True)')
...
from gan.gan import GanMDPModel
state_size = np.prod(env.observation_space.shape)
action_size = np.prod(env.action_space.shape)
gan_mdp_model = GanMDPModel(state_size=state_size, action_size=action_size, args=args)
...
# Memory
from utils.replay_memory import GanReplayMemory
memory = GanReplayMemory(args.replay_size, args.seed, state_dim=state_size, action_dim=action_size,
                         v_ratio=args.v_ratio, env_name=args.env_name, args=args, k_neighbours=10)
...
for i_episode in itertools.count(1):
    ...
    while not done:
        ...
        if args.gan and total_numsteps % args.update_env_model == 0:
            # Get real samples from environment
            batch_size = len(memory)
            o, _, _, _, _ = memory.sample_real(batch_size=batch_size)
            ...
            gan_mdp_model.train(inputs, labels, batch_size=256, holdout_ratio=0.2)
            ...
            o, a, _, _, _ = memory.sample_real(batch_size=args.n_rollout_samples)
            inputs = np.concatenate((o, a), axis=-1)
            inputs = gan_mdp_model.vae.decoder(torch.tensor(inputs, dtype=torch.float, device=gan_mdp_model.device))
            inputs = inputs.mean.detach()
            actions, _, _ = agent.policy.sample(inputs)
            inputs = torch.cat((inputs, actions.detach()), dim=-1).cpu().numpy()
            memory.rollout(inputs)
