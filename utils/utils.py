...
def get_neighboring_states(memory, state, action, env_name, n_neighbors=10):
    # Z-space
    position = len(memory)
    ...
    state, action = memory.buffer["state"][nn_self_idx], memory.buffer["action"][nn_self_idx]
    next_state, reward = memory.buffer["next_state"][nn_self_idx], memory.buffer["reward"][nn_self_idx]

    nn_state, nn_action = memory.buffer["state"][nn_indices], memory.buffer["action"][nn_indices]
    nn_next_state, nn_reward = memory.buffer["next_state"][nn_indices], memory.buffer["reward"][nn_indices]
