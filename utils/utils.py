...
def tensor_termination_fn(env_name, obs, act, next_obs):
    if env_name == "Hopper-v2":
        assert len(obs.shape) == len(next_obs.shape) == len(act.shape) == 2
        ...
        notdone = torch.isfinite(next_obs).all(dim=-1) \
                  * (torch.abs(next_obs[:, 1]) <= .2)
        done = ~notdone
        done = done[:, None]
        return done
    elif env_name == "InvertedDoublePendulum-v2":
        ...
        sin1, cos1 = next_obs[:, 1], next_obs[:, 3]
        sin2, cos2 = next_obs[:, 2], next_obs[:, 4]
        theta_1 = torch.atan2(sin1, cos1)
        theta_2 = torch.atan2(sin2, cos2)
        ...
        # HalfCheetah-v2 goes in here too
        assert len(obs.shape) == len(next_obs.shape) == len(act.shape) == 2
        return torch.zeros((len(obs), 1), dtype=torch.bool)
...
class TensorStandardScaler:
    ...
    def fit(self, X):
        ...
        self._sum_sq = (X ** 2).sum(dim=0)
        self._count = len(X)

        self._mean = self._sum / self._count
        squared_mean = self._sum_sq / self._count
        max_or_eps = torch.max(self._eps, squared_mean - torch.square(self._mean))
        self._std = torch.pow(max_or_eps, 0.5)
