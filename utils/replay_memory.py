...
from utils.utils import termination_fn, tensor_termination_fn
...
class BaseReplayMemoryTensor:
    def __init__(self, capacity, seed, state_dim, action_dim):
        ...
        self.buffer = {
            "state": torch.empty(size=(capacity, state_dim), dtype=torch.float, device=self.device),
            "next_state": torch.empty(size=(capacity, state_dim), dtype=torch.float, device=self.device),
            "action": torch.empty(size=(capacity, action_dim), dtype=torch.float, device=self.device),
            "reward": torch.empty(size=(capacity, 1), dtype=torch.float, device=self.device),
            "mask": torch.empty(size=(capacity, 1), dtype=torch.float, device=self.device),
        }
    ...
    def push(self, state, action, reward, next_state, done):
        self.buffer["state"][self.position] = torch.tensor(state, dtype=torch.float, device=self.device)
        self.buffer["next_state"][self.position] = torch.tensor(next_state, dtype=torch.float, device=self.device)
        self.buffer["action"][self.position] = torch.tensor(action, dtype=torch.float, device=self.device)
        self.buffer["reward"][self.position] = torch.tensor(reward, dtype=torch.float, device=self.device)
        self.buffer["mask"][self.position] = torch.tensor(done, dtype=torch.float, device=self.device)
...
class LocalClusterExperienceReplayClusterCenterTensor(BaseReplayMemoryTensor):
    def __init__(self, capacity, seed, state_dim, action_space, env_name="Hopper-v2", args=None, debug=False):
        super().__init__(capacity, seed, state_dim=state_dim, action_dim=np.prod(action_space.shape))
        ...
        from utils.utils import TensorMinibatchKMeans, TensorStandardScaler
        self.scaler = TensorStandardScaler()
        self.kmeans = TensorMinibatchKMeans(n_clusters=self.n_clusters, random_state=seed)
        self.clusters = [TensorStandardScaler() for _ in range(self.n_clusters)]
        ...
        self.max_action = torch.tensor(action_space.high, dtype=torch.float, device=self.device)
        self.min_action = torch.tensor(action_space.low, dtype=torch.float, device=self.device)
    ...
    def update_clusters(self, o, a, r, o_2):
        ...
        # max startup steps, else max max_timesteps
        labels = self.kmeans.labels_
        z_space = torch.cat((z_space, r, o_2), dim=-1)
        for n in range(len(labels)):
            self.clusters[labels[n]] = self.clusters[labels[n]].partial_fit(z_space[n].reshape(1, -1))
    ...
    def sample(self, batch_size):
        ...
        z_space = torch.cat((state, action), dim=-1)
        z_space_norm = self.scaler.transform(z_space)
        cluster_labels = self.kmeans.predict(z_space_norm)

        v_state = torch.empty(size=(batch_size, self.state_dim), dtype=torch.float, device=self.device)
        v_action = torch.empty(size=(batch_size, self.action_dim), dtype=torch.float, device=self.device)
        v_reward = torch.empty(size=(batch_size, 1), dtype=torch.float, device=self.device)
        v_next_state = torch.empty(size=(batch_size, self.state_dim), dtype=torch.float, device=self.device)
        for n in range(batch_size):
            mu, std = self.clusters[cluster_labels[n]].mean_, self.clusters[cluster_labels[n]].scale_ * 0.01

            zeros = torch.zeros(size=mu[:self.state_dim].shape, dtype=torch.float, device=self.device)
            ones = torch.ones(size=mu[:self.state_dim].shape, dtype=torch.float, device=self.device)
            v_state[n] = mu[:self.state_dim] + torch.normal(mean=zeros, std=ones) * std[:self.state_dim]

            zeros = torch.zeros(size=mu[self.state_dim:self.state_dim + self.action_dim].shape, dtype=torch.float, device=self.device)
            ones = torch.ones(size=mu[self.state_dim:self.state_dim + self.action_dim].shape, dtype=torch.float, device=self.device)
            v_action[n] = mu[self.state_dim:self.state_dim + self.action_dim] + torch.normal(mean=zeros, std=ones) * std[self.state_dim:self.state_dim + self.action_dim]

            zeros = torch.zeros(size=mu[self.state_dim + self.action_dim:self.state_dim + self.action_dim + 1].shape, dtype=torch.float, device=self.device)
            ones = torch.ones(size=mu[self.state_dim + self.action_dim:self.state_dim + self.action_dim + 1].shape, dtype=torch.float, device=self.device)
            v_reward[n] = mu[self.state_dim + self.action_dim:self.state_dim + self.action_dim + 1] + torch.normal(mean=zeros, std=ones) * std[self.state_dim + self.action_dim:self.state_dim + self.action_dim + 1]

            zeros = torch.zeros(size=mu[-self.state_dim:].shape, dtype=torch.float, device=self.device)
            ones = torch.ones(size=mu[-self.state_dim:].shape, dtype=torch.float, device=self.device)
            v_next_state[n] = mu[-self.state_dim:] + torch.normal(mean=zeros, std=ones) * std[-self.state_dim:]
        v_action = torch.max(torch.min(v_action, self.max_action), self.min_action)

        delta_state = (next_state - state).clone()
        v_delta_state = (v_next_state - v_state).clone()
